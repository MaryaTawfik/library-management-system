...
const createpayment = async (req, res) => {
  try {
    ...
    if (err.name === "ValidationError") {
      ...
      userFriendlyMessage =
        "The uploaded file is too large. Please upload a file smaller than 2MB.";
    } else if (err.message.includes("not found")) {
      userFriendlyMessage =
        "The specified user does not exist. Please check the user ID.";
    } else if (err.message.includes("Invalid payment amount")) {
      userFriendlyMessage =
        "Invalid payment amount. Please pay 300 for 1 month, 500 for 3 months, or 1000 for 1 year.";
    } else {
      userFriendlyMessage =
...
const updatepaymentstatus = async (req, res) => {
  ...
  try {
    const updatedPayment = await paymentService.updatePaymentAndMembership(
      id,
      status
    );
    ...
    if (err.message.includes("rejected")) {
      return res.status(400).json({
        message:
          "Your membership payment has been rejected. Please check your payment details and try again. If the issue persists, contact support for assistance.",
      });
    }
    res
      .status(500)
