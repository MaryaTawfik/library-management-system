...
// ✅ Mock fallback records (for dev if backend fails)
const MOCK_RECORDS = [
  {
    id: "r1",
...
export const getAllBorrows = async () => {
  try {
    ...
    console.warn("borrowService.getAllBorrows failed, returning mock", err.message);
    return MOCK_RECORDS;
  }
};
...
export const createPayment = async (userId, bankTransactionID, amount, file = null) => {
  try {
    ...
    const res = await axios.post(`${API_URL}/payments`, formData, {
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "multipart/form-data",
      },
...
export const getPendingPayments = async () => payments;
export const approvePayment = async (id) => {
  payments = payments.map((p) => (p._id === id ? { ...p, status: "Approved" } : p));
  return { success: true };
};
...
export const updateUserProfile = async (payload) => {
  try {
    ...
    let response;
    if (payload instanceof FormData) {
      response = await axios.put(`${API_URL}/auth/profile`, payload, { headers });
    } else {
      response = await axios.put(`${API_URL}/auth/profile`, payload, {
        headers: { ...headers, "Content-Type": "application/json" },
      });
